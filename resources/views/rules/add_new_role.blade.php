...
<style>
    ...
    .choices__list--dropdown, .choices__list[aria-expanded]{
        background-color: #070d2a !important;
    }
</style>
</head>
...
@endif
              ...
              <div class="card">
                 <div class="card-body">
                  ...
                  @enderror  --}}
                      ...
                      <div class="mb-4">
                        <h5 class="mb-3">الاوامر</h5>
                        <select name="permissions[]" id="permissions-select" multiple>
                   ...
                   {{--  <div class="mb-4">
                      <h5 class="mb-3">Guard Name</h5>
                      <input type="text" name="guard_name" class="form-control" placeholder="write title here....">
                   </div>  --}}
                   <div class="my-4">
                    <button class="btn btn-primary">إضافه</button>
                   </div>
                    </form>
                 </div>
              </div>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
        const permissionsSelect = new Choices("#permissions-select", {
            removeItemButton: true,
            allowHTML: true
        });

        const selectElement = document.getElementById("permissions-select");
        ...
        selectElement.addEventListener("change", function () {
            ...
            if (selectedValues.includes("all")) {
                ...
                permissionsSelect.setChoiceByValue([...selectedValues, ...Array.from(this.options).map(o => o.value)]);
            }
            // If "Select All" is unchecked, deselect all
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
        const permissionsSelect = new Choices('#permissions-select', {
            removeItemButton: true, // Enable removing items with an "x"
            placeholder: true,
            placeholderValue: "Select permissions...",
            searchEnabled: true, // Allow searching for permissions
            shouldSort: false // Prevent sorting of options
        });

        const selectAllOption = document.querySelector('#permissions-select option[value="all"]');

        // Handle the change event on the select element
        document.querySelector('#permissions-select').addEventListener('change', function (event) {
            const selectedOptions = Array.from(event.target.selectedOptions).map(option => option.value);

            // If "Select All" is selected
            if (selectedOptions.includes('all')) {
                // Select all permissions
                const allPermissions = @json($permissions->pluck('id')->toArray());
                permissionsSelect.setValue([...allPermissions, selectAllOption.value]);

            } else {
                // If "Select All" is deselected, deselect all items
                if (!selectedOptions.includes('all')) {
                    permissionsSelect.removeActiveItems();
                }
            }

            // If any permission other than "Select All" is selected, deselect "Select All"
            if (selectedOptions.length > 0 && selectedOptions[0] !== 'all') {
                permissionsSelect.removeActiveItems([selectAllOption.value]);
            }
        });
    });
</script>
...
<script>
    ...
    @foreach (['imageError', 'nameError', 'emailError', 'phoneError', 'addressError', 'passwordError', 'genderError'] as $field)
        @if ($errors->has($field))
