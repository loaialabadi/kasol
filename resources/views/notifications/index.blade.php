      ...
      <div style="justify-content: space-between;" class="row g-3">
        <div class="col-auto">
          <div class="position-relative">
           <a href="{{route('create_notification')}}" class="btn btn-primary">إضافه</a>
          </div>
        </div>
    ...
    <tbody>
        @foreach ($notes as $note)
            <tr>
                ...
                <td>{{$note->content}}</td>
                <td>{{$note->type}}</td>
                <td>{{isset($note->user)?$note->user->name:''}}</td>
                <td>
                    <div class="dropdown">
                            ...
                            <li>
                                <a class="dropdown-item" href="javascript:void(0);"
                                    onclick="openOrderDetailsModal(
                                        '{{ $note->id }}',
                                        '{{ optional($note->user)->name ?? 'غير متوفر' }}',
                                        '{{ $note->total_price ?? 'غير متوفر' }}',
                                        '{{ $note->payment_method ?? 'غير متوفر' }}',
                                        '{{ $note->receiving_method ?? 'غير متوفر' }}',
                                        '{{ $note->shipping_address ?? 'غير متوفر' }}',
                                        '{{ $note->status ?? 'غير متوفر' }}'
                                    )">
                                    عرض التفاصيل
                                </a>
                            </li>
...
<script>
  ...
  function openOrderDetailsModal(id, userName, totalPrice, paymentMethod, receivingMethod, shippingAddress, status) {
