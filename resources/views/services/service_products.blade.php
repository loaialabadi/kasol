      ...
      <div class="row g-3">
        <div class="col-auto">
          <div class="position-relative">
            <!--<form action="{{ route('search_offer',['id'=>$id]) }}" method="GET" class="d-flex">-->
            <!--    <input class="form-control px-5" type="search" name="search" placeholder="بحث" value="{{ request('search') }}">-->
            <!--    <input class="form-control px-5" type="hidden"  name="category_id" placeholder="بحث" value="{{ $id }}">-->
        ...
        <div class="col-auto">
          <div class="d-flex align-items-center gap-2 justify-content-lg-end">
            ...
            <a class="btn btn-primary px-4" href="{{ route('add_new_ser_prod',['id'=>$id]) }}">
                <i class="bi bi-plus-lg me-2"></i>إضافه 
            </a>
                    <button class="btn btn-primary" data-bs-target="#add" data-bs-toggle="modal"> <i class="fas fa-add"></i> اضافة ملف منتجات</button>
            ...
            <div class="table-responsive white-space-nowrap">
              <table class="table align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <img style="width:100px;height:100px" src="{{ asset('storage/' . $product->image) }}" alt="img"/>
                      ...
                      <td>
                          <!--<div>{{json_encode($product->sub_category)}}</div>-->
                        <div class="d-flex align-items-center gap-3">
                        <p>{{isset($product->sub_category)? $product->sub_category->name:'' }}</p>
                        </div>
                      </td>
                      
                      <td>
                          <a href="{{route('prod_sizes',['id'=>$product->id])}}" class="btn btn-primary">الأحجام</a>
                      </td>
                    ...
                    <!--                <img src="{{ asset('path/to/active-image.png') }}" alt="active"/>-->
                    <!--            @else-->
                    <!--                <img src="{{ asset('path/to/not-active-image.png') }}" alt="not active"/>-->
                    <!--            @endif-->
                    <!--        </div>-->
                    <!--    </div>-->
                    <!--</td>-->
                    <!--<td><a class="btn btn-primary" href="{{ route('change_offer_status',['id'=>$product->id]) }}">تغيير الحاله</a></td>-->
                      <td>
                        <div class="dropdown">
                          ...
                          <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item" href="javascript:void(0);" onclick="openDeleteModal({{ $product->id }})">حذف</a>
                            </li>
                                                        <li><a class="dropdown-item" href="{{ route('update_product', $product->id) }}">تحديث</a></li>
                            {{--  <li><a class="dropdown-item" href="{{ route('roles_pemissions', $product->id) }}">Role Permissions</a></li>  --}}
                            <a class="dropdown-item" href="javascript:void(0);" onclick="openOfferModal({{ $product }})">
                                عرض التفاصيل
                            </a>
                        </ul>
                        </div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
             ...
             <form method="post" action="{{ route('up_prod_file',['id'=>$id]) }}" enctype="multipart/form-data">
    @csrf
    <div class="container">
        <div class="row my-4">
            <div class="col-md-12 my-2">
                <label>الملف</label>
                <input type="file" class="form-control @error('file') is-invalid @enderror" name="file" value="{{ old('file') }}" placeholder="أدخل السؤال">
                @error('file')
    ...
    <div class="modal-footer">
      ...
      <div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="confirmDeleteModalLabel">تأكيد حذف</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              هل تريد حذف هذا
      ...
      <div class="modal fade" id="offerModal" tabindex="-1" aria-labelledby="offerModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div id="offerImages" class="mb-3">

                    </div>
                    {{--  <img src="{{ asset('assets/images/offer_images/') }}" alt="">  --}}
                    <p><strong>الوصف:</strong> <span id="offerDescription"></span></p>
                    <p><strong>السعر الاصلى:</strong> $<span id="offerPrice"></span></p>
                </div>
                <div class="modal-footer">
...
<script>
    ...
    function openOfferModal(offer) {
        document.getElementById('offerModalLabel').innerText = offer.title;
        ...
        document.getElementById('offerDiscountedPrice').innerText = offer.descounted_price;

        let imagesContainer = document.getElementById('offerImages');
        imagesContainer.innerHTML = ''; // Clear previous images

        if (offer.images && offer.images.length > 0) {
            offer.images.forEach(image => {
                let imgElement = document.createElement('img');
                imgElement.src = "{{ asset('storage') }}"+"/" + image.image; // Adjust path as needed
                imgElement.classList.add('img-fluid', 'rounded', 'me-2'); // Styling
...
<script>
    ...
    // Function to open the modal and set the offer ID for deletion
    ...
    $('#confirmDeleteButton').on('click', function () {
        if (categoryIdToDelete) {
            // Redirect to delete route with the offer ID
            window.location.href = "{{ route('delete_product', '') }}" + '/' + categoryIdToDelete;
        }
    });
